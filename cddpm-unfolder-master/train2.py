...
if configs.load_preprocessed_training_data:
    x = np.load(configs.input_path + "train_truth_" + configs.train_type + configs.boson + ".npy", mmap_mode='r')[:,1:]
    y = np.load(configs.input_path + "train_reco_" + configs.train_type + configs.boson + ".npy", mmap_mode='r')[:,1:]
...
for iteration in range(1, configs.epochs+1):
    ...
    if iteration == 5 or iteration % configs.save_int == 0:
        ...
        torch.save(cDDPM.state_dict(), configs.ckpt_path + 'gen_' + configs.train_type + configs.boson + '_b' + str(batch_size) + '_it' + str(iteration) + '.pth')
