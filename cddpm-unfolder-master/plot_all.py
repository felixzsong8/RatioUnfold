...
unfold = np.load("./outputs/jetsW/unfold_" + unf_type + boson[0] + ".npy")[:,1:]
...
unfoldZ = np.load("./outputs/jetsZ/unfold_" + unf_type + boson[1] + ".npy")[:,1:]
...
for i in range(4):
    if i == 0:
        ...
        bins = np.array([30.,40.,50.,60.,70.,80.,90.,100.,120.,140.,160.,180.,200.,220.,240.,260.,280.,300.,350.,400.,450.,500.,600.,700.,800.,900.,1000.])
   ...
   # plt.close()
    ...
    a5.hist(ones, bins=bins, weights=ratio_division, histtype="step", label='Unfolded / Truth ', linewidth=1.5, color='blue')
    ...
    if i == 0:
        print(plots_path + unf_type)
        file = uproot.open("unfolding_results.root")
        file2 = uproot.open("BBBunfold.root")
        f3, a6 = plt.subplots()

        #Calculate Quantities to compare between methods
        hRatioUnfoldedBin, _ = file["hRatioUnfoldedBin;1"].to_numpy()
        hRatioUnfoldedSVD, _ = file["hRatioUnfoldedSVD;1"].to_numpy()
        hTruth, _ = file["hRatioTruth;1"].to_numpy()
        BBBunfold, _ = file2["unfolded;1"].to_numpy()
        BBBtruth, _ = file2["truth;1"].to_numpy()
        uncertainty, _ = file2["uncertainty;1"].to_numpy()
        wass_ML = wasserstein_distance(ratio_truth_all, ratio_unfold_all)
        wass_matrix = wasserstein_distance(hTruth, hRatioUnfoldedBin)
        wass_SVD = wasserstein_distance(hTruth, hRatioUnfoldedSVD)
        wass_Bin = wasserstein_distance(BBBtruth, BBBunfold)

        chi2_Bin = np.sum((BBBunfold - BBBtruth)**2)
        chi2_Matrix = np.sum((hRatioUnfoldedBin - hTruth)**2)
        chi2_SVD = np.sum((hRatioUnfoldedSVD - hTruth)**2)
        ratio_unfold_all = np.array(ratio_unfold_all)
        chi2_ML = np.sum((ratio_unfold_all - ratio_truth_all)**2)


        # Extract final ratio histograms from the ROOT files
        hFinalRatioBin = file["hFinalRatioBin;1"].to_numpy()
        hFinalRatioSVD = file["hFinalRatioSVD;1"].to_numpy()
        BBBfinal = file2["MeasDivTruth;1"].to_numpy()

        # Extract bin contents and bin edges
        
        final_ratio_bin_values = hFinalRatioBin[0]  # Y-values
        final_ratio_svd_values = hFinalRatioSVD[0]  # Y-values
        BBBunfold_values = BBBfinal[0]  # Y-values
        bin_edges = hFinalRatioBin[1]  # X-axis bin edges

        # Compute bin centers for plotting
        bin_centers = 0.5 * (bin_edges[:-1] + bin_edges[1:])

        # Overlay the final ratio distributions on the ratio subplot
        a6.hist(ones, bins=bins, weights=ratio_division, histtype="step", label='cDDPM', linewidth=1.5, color='blue')
        a6.hist(bin_centers, bins=bin_edges, weights=final_ratio_bin_values, 
                histtype="step", label='Ratio Matrix', linewidth=1.5, color='green')
        a6.hist(bin_centers, bins=bin_edges, weights=final_ratio_svd_values, 
                histtype="step", label='Ratio SVD', linewidth=1.5, linestyle="dashed", color='orange')
        a6.hist(bin_centers, bins=bin_edges, weights=BBBunfold_values, 
                histtype="step", label='Bin-By-Bin', linewidth=1.5, linestyle="dotted", color='red')

        # Add horizontal reference line at y = 1
        a6.axhline(y=1, color='black', linestyle='dashed', linewidth=1)
        a6.set_xlim(0, 600)
        a6.legend(loc='upper right')
        a6.set_ylim(0.7, 1.4)
        a6.set_ylabel('Unfolded / Truth')
        a6.set_xlabel(axis_label[i])
        a6.set_title("Methods Distribution Comparison")
        f3.savefig("MethodComparison.png")
        ...
        sum_SVD = 0
        sum_BBB = 0
        for j in range(len(ratio_division)-4):
            sum_ML += abs(ratio_division[j] - 1)
            sum_matrix += abs(final_ratio_bin_values[j] - 1)
            sum_SVD += abs(final_ratio_svd_values[j] - 1)
            sum_BBB += abs(BBBunfold_values[j] - 1)


        print(f"Displacement sum_ML: {sum_ML}")
        print(f"Displacement sum_matrix: {sum_matrix}")
        print(f"Displacement sum_SVD: {sum_SVD}")
        print(f"Displacement sum_BBB: {sum_BBB}")

        print(f"Wasserstein ML: {wass_ML}")
        print(f"Wasserstein Matrix: {wass_matrix}")
        print(f"Wasserstein SVD: {wass_SVD}")
        print(f"Wasserstein Bin: {wass_Bin}")
        chi_char = '\u03C7'
        print(f"chi_char^{2} ML: {chi2_ML}")
        print(f"chi_char^{2} Matrix: {chi2_Matrix}")
        print(f"chi_char^{2} SVD: {chi2_SVD}")
        print(f"chi_char^{2} Bin: {chi2_Bin}")
