...
for k in range(len(distribution)):
    for i in range(len(type)):    
        ...
        print(len(event_num))
        lepton_count = data_full['lepton_count']
        truth = data_full['gen_jets']
        reco = data_full['reco_jets']

        data = np.append(truth, reco, axis=1)
        data = np.append(lepton_count[:,np.newaxis], data, axis=1)
        ...
        truth = np.append(lepton_count[:,np.newaxis], truth, axis=1)
        reco = np.append(event_num[:,np.newaxis], reco, axis=1)
        reco = np.append(lepton_count[:,np.newaxis], reco, axis=1)
